...
n_episodes = 10000
print_every=100
reward_type = 'Step_Bonus' # 'Terminal' 'Step' 'Step_Bonus'
problem_batch = problemLoader(max_shifts)
print(f"unique problems: {len(problem_batch)}")
filename = f"{now}_random" #seed{seed}_eps{n_episodes}_ms{max_shifts}_batchlen{len(problem_batch)}_rt{reward_type}"
 #f"Random_" 
